...
#define     UARTx_TXD_PIN       GPIO_Pin_3
#define     UARTx_TXD_AF        PC3_TXD0

// RXD
#define     UARTx_RXD_PORT      GPIOC
#define     UARTx_RXD_PIN       GPIO_Pin_4
#define     UARTx_RXD_AF        PC4_RXD0

// Uart定义
#define     UARTx_BAUDRATE      (9600)              // 波特率
#define     UARTx               UART0               // Uart Base
#define     UARTx_IRQn          UART0_IRQn          // IRQ
#define     UARTx_Handler       UART0_Handler       // 中断处理程序
#define     UARTx_TDR           UART0->TDR          // 发送数据寄存器
#define     UARTx_RDR           UART0->RDR          // 接收数据寄存器
#define     UARTx_CLOCK         RCC_APB1_UART0      // 时钟
...
///  直接操作寄存器位控制 参考
//		UART0->CR.BIT.TEN=0; //发送使能关闭
//		UART0->CR.BIT.TEN=1; //发送使能打开	
//		UART0->CR.BIT.REN=0; //接收使能关闭
//		UART0->CR.BIT.REN=1; //接收使能打开
}
...
ST_UartT     gSUart0Test;

#define     sUart   gSUart0Test
...
void    UART0_Test(void)
{
    ...
    if( sUart.tranProcess == 0 )                // 非发送中才允许去更新发送内容
    {
        if( testIdx != receIdx )
        {
            ...
            // 启动发送
						UART0->FR.BIT.TCC = 1;      // 清标志位
            UART0->TDR.BIT.TDR  = sUart.tranBuf[0];
...
// RI只能通过对RDR读操作来清零  每次一定要读取缓存，取走数据
      //  recv_data = UART0->RDR.BIT.RDR;
        ...
        sUart.receBuf[sUart.receIdx] = UART0->RDR.BIT.RDR;
    ...
    if (UART0->FR.BIT.TC)
    {
        ...
        {
            UART0->TDR.BIT.TDR = sUart.tranBuf[sUart.tranIdx];
            sUart.tranIdx ++;
        }
        else
        {
		...
		{						
			...
			//2:  要清接收溢出标志位
			UART_ReceiveData(UART0);	// 读取一次接收缓存				
			UART0->FR.BIT.RXOVC =1;	 //清接收溢出标志位
		}
}
//测试 DEMO——————————————————————————————————————————
/*----------------------------------------- E.N.D -----------------------------------------*/
